...
class AppStore extends Component {
  state = {selecttabId: tabsList[0].tabId, inputsearch: ''}

  oncliktabitem = tabId => this.setState({selecttabId: tabId})

  onenterinput = event => this.setState({inputsearch: event.target.value})

  filterapplist = () => {
    const {selecttabId, inputsearch} = this.state
    const searcinputfilter = appsList.filter(eachone =>
      eachone.appName.toUpperCase().includes(inputsearch.toUpperCase()),
    )

    const filterapplistis = searcinputfilter.filter(
      eachone => eachone.category === selecttabId,
    )

    return filterapplistis
  }

  render() {
    const {selecttabId} = this.state
    const appsListResult = this.filterapplist(appsList)
    return (
      <div className="appstore-main-bg">
        <div className="appstore-contenet-con">
          <h1 className="heading">App Store</h1>
          <div className="searcn-container">
            <input
              type="search"
              className="search"
              placeholder="search"
          ...
          <ul className="tabscontainer">
            {tabsList.map(eachtab => (
              <TabItem
                eachtab={eachtab}
                key={eachtab.tabId}
                oncliktabitem={this.oncliktabitem}
                isActive={eachtab.tabId === selecttabId}
              />
            ))}
          </ul>
          <ul className="appcontainer">
            {appsListResult.map(eachapp => (
